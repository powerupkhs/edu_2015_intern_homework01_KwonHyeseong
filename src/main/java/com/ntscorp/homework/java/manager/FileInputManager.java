...
public class FileInputManager implements DataManager {
	...
	public Map<String, Car> readList(String folderPath) {
		...
		for (int i = 0; i < fileList.length; i++) {
			...
			Map<String, Car> inputCars = readListFromFile(folderPath, fileName);
	...
	public Map<String, CarOrder> readOrder(String folderPath) {
		...
		for (int i = 0; i < fileList.length; i++) {
			String fileName = fileList[i].getName();
			CarOrder newCarOrder = readOrderFromFile(folderPath, fileName);
	 ...
	 */
	public Map<String, Car> readListFromFile(String folderPath, String fileName) {
		...
		try {
			input = new BufferedReader(new InputStreamReader(new FileInputStream(folderPath + "/" + file), "EUC-KR"));
	 ...
	 * @return CarOrder 객체
	 */
	public CarOrder readOrderFromFile(String folderPath, String fileName) {
		BufferedReader input = null;
		String read;
		...
		try {
			...
			while (read != null) {
				...
				CarOrderInfo newCarOrderInfo = new CarOrderInfo(Double.parseDouble(fileValue[1]));
